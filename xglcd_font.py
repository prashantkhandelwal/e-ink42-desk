...
class XglcdFont(object):
    ...
    def __load_xglcd_font(self, path):
        ...
        with open(path, 'r') as f:
            for line in f:
                line = line.strip()
                if len(line) == 0 or not (
                        line.startswith('0x') or line.startswith('$')):
                    continue
                comment = line.find('//')
                basic_comment = line.find("'")
                if basic_comment != -1 and (
                        comment == -1 or basic_comment < comment):
                    comment = basic_comment
                if comment != -1:
                    line = line[0:comment].strip()
                if line.endswith(','):
                    line = line[0:len(line) - 1]
                values = line.replace('$', '0x').split(',')
                mv[offset: offset + bytes_per_letter] = bytearray(
                    int(value.strip(), 16) for value in values)
